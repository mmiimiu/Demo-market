...
export function useProfileData(userId: string | null, lang: Language): ProfileData | null {
  ...
  return useMemo(() => {
    ...
    if (currentUser && (userId === currentUser.uid || userId === 'current_user')) {
      const mockStr = typeof window !== 'undefined' ? localStorage.getItem('prime_mock_user') : null;
      const mockObj = mockStr ? JSON.parse(mockStr) : null;
      const mockKyc = (typeof window !== 'undefined' ? localStorage.getItem('primerent_mock_kyc') : null) || 'unverified';
      const role = mockObj?.role || (typeof window !== 'undefined' ? localStorage.getItem('primerent_user_role') : null) || 'renter';
      const isAgentVerified = mockObj?.verified?.agent === true || (typeof window !== 'undefined' && localStorage.getItem('primerent_agent_verified') === 'true');
      return {
        uid: currentUser.uid,
        ...
        role, kycStatus: mockKyc as any, location: mockObj?.location || (isTh ? 'กรุงเทพฯ' : 'Bangkok'),
        isAgentVerified,
  ...
  }, [userId, lang, currentUser?.uid, currentUser?.displayName, currentUser?.email, currentUser?.photoURL]);
}
