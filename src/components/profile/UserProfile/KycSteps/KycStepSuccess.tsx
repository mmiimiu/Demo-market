...
export function KycStepSuccess({ lang, onClose, kycFullName = '', kycIdNumber = '', kycDocType = 'thaid' }: KycStepSuccessProps) {
  ...
  const isAgentRole = userRole === 'agent'; // CRD is strictly for Agent role only
  ...
  const idLabel = isPassport ? (isTh ? 'หมายเลขหนังสือเดินทาง' : 'Passport Number') : (isTh ? 'เลขบัตรประชาชน' : 'Citizen ID');
  const displayIdNumber = isPassport && (!kycIdNumber || kycIdNumber.length === 13) ? 'AA1234567' : kycIdNumber;
  ...
  return (
    <div className="space-y-4 text-center animate-in zoom-in-95 duration-500 pb-2 font-sans">
      ...
      <div className="space-y-1">
        ...
        <p className="text-[11px] font-bold text-gray-500 max-w-sm mx-auto leading-relaxed">
          {isTh 
            ...
            : `Your identity has been fully verified using digital ID gateways (${methodLabel}).`}
        </p>
      </div>

      {/* CRD Criminal Record Verification Box (Shown ONLY for Agent role) */}
      {isAgentRole && (
        <>
          <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 text-left space-y-3 max-w-sm mx-auto">
            <div className="flex items-center justify-between border-b border-slate-100 pb-2">
              ...
              <span className="text-[10px] bg-red-100 text-red-800 font-bold px-2 py-0.5 rounded-full">
                {isTh ? 'จำเป็นสำหรับเอเจ้นท์' : 'Agent Required'}
              </span>
            </div>

            <p className="text-[11px] font-semibold text-slate-500 leading-relaxed">
              {isTh 
                ? `สำหรับบทบาทเอเจ้นท์ กรุณาคลิกปุ่มด้านล่างเพื่อยื่นตรวจประวัติอาชญากรรมผ่านกองทะเบียนฯ ด้วยข้อมูลจาก ${methodLabel}` 
                : `For Agent role, please click the link below to process criminal record verification using your verified ${methodLabel} credentials.`}
            </p>
            ...
            <div className="space-y-2 text-xs">
              <div className="flex items-center justify-between bg-white p-2 rounded-xl border border-slate-100">
                ...
                <button onClick={() => handleCopy(kycFullName, 'name')} className="text-gray-400 hover:text-primary transition-colors p-1" title={isTh ? 'คัดลอกชื่อ' : 'Copy Name'}>
                  {copiedName ? <Check className="w-4 h-4 text-green-600" /> : <Copy className="w-4 h-4" />}
                </button>
              </div>

              <div className="flex items-center justify-between bg-white p-2 rounded-xl border border-slate-100">
                <div><p className="text-[10px] text-gray-400 font-bold">{idLabel}</p><p className="font-bold text-slate-800">{displayIdNumber}</p></div>
                <button onClick={() => handleCopy(displayIdNumber, 'id')} className="text-gray-400 hover:text-primary transition-colors p-1" title={isTh ? 'คัดลอกเลขประจำตัว' : 'Copy ID'}>
                  {copiedId ? <Check className="w-4 h-4 text-green-600" /> : <Copy className="w-4 h-4" />}
                </button>
              </div>
            </div>
            <div className="bg-emerald-50/80 border border-emerald-100 rounded-xl p-2.5 text-[10px] text-emerald-800 flex items-start gap-1.5 font-semibold">
              <span className="text-xs leading-none">✨</span>
              <p>{isTh ? 'แพลตฟอร์มสำรองจ่ายค่าตรวจประวัติ ฿100 ให้ก่อน โดยระบบจะนำไปหักคืนจากค่าคอมมิชชั่นปล่อยเช่าครั้งแรกของคุณอัตโนมัติ' : 'Platform advances the ฿100 fee, automatically recovered from your first successful lease commission.'}</p>
            </div>
            <a href="https://crd.go.th/crd-service-1/" target="_blank" rel="noopener noreferrer" onClick={() => { setHasClickedCrd(true); setCrdSyncStatus('linking'); if (typeof window !== 'undefined') localStorage.setItem('primerent_pending_crd_deduction', 'true'); setTimeout(() => setCrdSyncStatus('success'), 3000); }} className="flex items-center justify-center gap-1.5 w-full bg-[#1e293b] hover:bg-slate-900 text-white font-black text-xs py-2.5 rounded-xl transition-all shadow-sm active:scale-95">
              <span>{isTh ? 'ไปที่เว็บไซต์ตรวจสอบประวัติ (CRD)' : 'Go to CRD Portal'}</span><ExternalLink className="w-3.5 h-3.5" />
            </a>
          </div>
          {crdSyncStatus === 'linking' && (
            <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 text-[11px] font-bold text-blue-700 flex items-center gap-2 animate-pulse justify-center max-w-sm mx-auto">
              <span className="w-2 h-2 rounded-full bg-blue-600 animate-ping"></span><span>{isTh ? '🔗 กำลังเชื่อมโยงข้อมูลผลการตรวจสอบจากระบบ CRD...' : '🔗 Connecting record check results from CRD...'}</span>
            </div>
          )}
      ...
      <div className="pt-2 flex justify-center">
        <Button 
          onClick={onClose} 
          disabled={isAgentRole && (!hasClickedCrd || crdSyncStatus === 'linking')}
