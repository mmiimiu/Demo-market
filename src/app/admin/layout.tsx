...
export default function AdminLayout({
  ...
  // Guard Clause: strictly enforce that only admin/superadmin roles can access the layout children
  const isAdmin = userRole === 'admin' || userRole === 'superadmin' || userRole === 'sa';
  if (!isAdmin) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col">
        <Navbar scrolled={true} portal="admin" />
        <div className="flex flex-col items-center justify-center flex-1 pt-[80px] px-6 text-center font-thai">
          <div className="bg-red-50 border border-red-100 rounded-3xl p-8 max-w-md shadow-xl shadow-red-100/10 flex flex-col items-center">
            ...
            <p className="text-sm text-gray-500 font-bold leading-relaxed mb-6">
              หน้านี้สงวนไว้สำหรับบัญชีที่มีบทบาท <span className="text-red-600 font-black">ผู้ดูแลระบบ (Admin/SuperAdmin)</span> เท่านั้น กรุณาสลับสิทธิ์หรือลงชื่อเข้าใช้ด้วยบัญชีแอดมินจำลอง
            </p>
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      <Navbar scrolled={true} portal="admin" />
      <div className="flex flex-1 h-[calc(100vh-80px)] bg-slate-50 font-thai pt-[80px]">
        <AdminSidebar />
