...
import { getProjectDir, scanTodos, getApiRoutes, checkEnvVars, getDbSchema, runLinter, checkVulnerabilities } from './project-scanner.js';
// ─── SERVER INIT ───────────────────────────────────────────────────────────
const server = new Server({ name: 'direction-mcp', version: '1.0.0' }, { capabilities: { tools: {}, resources: {} } });
// ─── HELPERS ───────────────────────────────────────────────────────────────
const ok = (data) => ({
...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        ...
        {
            name: 'get_direction',
            description: 'Get the FULL project direction: all sections, decisions, active tasks, recent notes. ' +
        ...
        {
            name: 'set_section',
            description: 'Create or update a named context section. ' +
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
    ...
    switch (name) {
        ...
        case 'get_direction': {
            ...
            const tasks = db.prepare("SELECT * FROM tasks WHERE status != 'done' ORDER BY CASE priority WHEN 'high' THEN 0 WHEN 'medium' THEN 1 ELSE 2 END, created").all();
      ...
      `).run(n, content);
        ...
        case 'get_section': {
            ...
            if (!row)
                throw new McpError(ErrorCode.InvalidRequest, `Section "${n}" not found`);
            return ok(row);
        }
        case 'list_sections': {
        ...
        // ── Decisions ─────────────────────────────────────────────────────────
        case 'add_decision': {
            const { title, choice, reasoning, status = 'decided' } = args;
            const r = db.prepare('INSERT INTO decisions (title, choice, reasoning, status) VALUES (?, ?, ?, ?)').run(title, choice, reasoning, status);
        ...
        case 'add_task': {
            ...
            const r = db.prepare('INSERT INTO tasks (title, description, priority) VALUES (?, ?, ?)').run(title, description, priority);
            return ok({ ok: true, id: r.lastInsertRowid });
        }
        case 'update_task': {
            const { id, status, priority, title } = args;
            if (status)
                db.prepare("UPDATE tasks SET status = ?, updated = datetime('now') WHERE id = ?").run(status, id);
            if (priority)
                db.prepare("UPDATE tasks SET priority = ?, updated = datetime('now') WHERE id = ?").run(priority, id);
            if (title)
                db.prepare("UPDATE tasks SET title = ?, updated = datetime('now') WHERE id = ?").run(title, id);
            return ok({ ok: true, id });
        }
        case 'list_tasks': {
            const { status } = args;
            const rows = status
                ? db.prepare("SELECT * FROM tasks WHERE status = ? ORDER BY CASE priority WHEN 'high' THEN 0 WHEN 'medium' THEN 1 ELSE 2 END, created").all(status)
                : db.prepare("SELECT * FROM tasks ORDER BY CASE status WHEN 'in_progress' THEN 0 WHEN 'todo' THEN 1 ELSE 2 END, CASE priority WHEN 'high' THEN 0 WHEN 'medium' THEN 1 ELSE 2 END").all();
            return ok(rows);
        }
        // ── Notes ─────────────────────────────────────────────────────────────
        case 'add_note': {
            const { content, tag = '' } = args;
