...
import { EarlyTerminationModal } from './EarlyTerminationModal';
...
export function LeaseHero({ lease, daysLeft, isTh, label }: LeaseHeroProps) {
  ...
  return (
    <>
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-3 flex-wrap">
                ...
                <button 
                  ...
                  className="bg-white/20 hover:bg-white/30 text-white text-[9px] font-bold rounded-xl px-2.5 py-0.5 border border-white/30 transition-all cursor-pointer hover:scale-105"
                >
                  {label('⚡ ขอเลิกสัญญาก่อนกำหนด (Early Termination)', '⚡ Early Termination', '⚡ 提前解约')}
                </button>
              </div>
            </div>
          </div>
      ...
      {/* Security-Enforced Early Termination Modal */}
      <EarlyTerminationModal 
        isOpen={isTerminationModalOpen}
        onClose={() => setIsTerminationModalOpen(false)}
        lang={isTh ? 'th' : 'en'}
        propertyName={isTh ? lease.propertyName : lease.propertyNameEn}
        depositAmount={36000}
      />
    </>
  );
