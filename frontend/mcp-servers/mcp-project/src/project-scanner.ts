...
export function getProjectDir(): string {
  const row = db.prepare("SELECT content FROM sections WHERE name = 'project_dir'").get() as { content: string } | undefined;
  if (!row) {
    return process.cwd();
  }
  return row.content;
...
export function scanTodos(targetDir: string, syncToTasks = false): TodoItem[] {
  ...
  for (const file of files) {
    try {
      ...
      if (!content.includes('TODO') && !content.includes('FIXME')) continue;
...
export interface ApiRoute {
  path: string;
  file: string;
  type: 'Next.js App' | 'Next.js Pages' | 'Express / Custom';
}

export function getApiRoutes(targetDir: string): ApiRoute[] {
...
export interface EnvVarReport {
  ...
  inExample: boolean;
  inCode: boolean;
  isSetLocally: boolean;
}

export function checkEnvVars(targetDir: string): EnvVarReport[] {
  const envVars = new Map<string, Partial<EnvVarReport>>();
  ...
  const localEnvVars = new Set<string>();
  ...
  for (const line of localLines) {
    ...
    if (trimmed && !trimmed.startsWith('#')) {
      ...
      if (match) {
        ...
        if (val) localEnvVars.add(name);
      }
    }
  }

  const report: EnvVarReport[] = [];
...
export function getDbSchema(targetDir: string): DbSchemaInfo | null {
  const schemaFiles = [
...
export async function runLinter(targetDir: string): Promise<{ success: boolean; output: string }> {
...
export async function checkVulnerabilities(targetDir: string): Promise<{ output: string }> {
  const pkgPath = path.join(targetDir, 'package.json');
  if (!fs.existsSync(pkgPath)) {
